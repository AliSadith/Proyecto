...
from app.services.user import get_user_by_emsil

router = APIRouter()


@router.post("/login")
async def login (auth:Auth, db: Session = Depens(get_db)):
    try:
        ...
        validation_password = squals_password(auth.password, hash_password=user.hashed_password)
        ifvalidation_password is False:
        return JSONResponse(content=("message": "credenciales invalidas")
                                status_code=status.HTTP_401_UNAUTHORIZED)
    
    roles = [role.nombre for role in user.roles]

    toeknn = create_access_token({"username": user.email, "role": roles, "id": user.id})
    return {"token": token}
