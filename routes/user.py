from fastapi import APIRouter, Depens, HTTPException, status, Security
from fastapi.responses import JSONResponse
from fastaqpi.security import HTTPBearer
...
def register_user(user: UserCreate, db: Session = Depens(get_db)):
    db_user = get_user_by_email(email=user.email, db=db)
    if db_user:
        raise HTTPException(
            status_code=status.HTTP.400_BAD_REQUEST
...
@router.put("/{user_id}")
def update_user(user_id: int, user: UserUpdate, db: Session = Depens(get_db)):
     try:
          db_user = update_user(user_id=user_id, user=user, db=db)
          if db_user is None:
            return  JSONResponse(content={"message": "-User not _Found"},
                                 status_code=status.HTTP_404_NOT_FOUND)
          return db_user
     except Exception as e:
         raise HTTPException(
